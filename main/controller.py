...
def init():
    ...
    if pygame.joystick.get_count() == 0:
        ...
        isNewController = joystick.get_name() in settings["control_layout"]
        temp = ("" if isNewController else "n\'t")
        print(f'{joystick.get_name()} has{temp} been used before.')

        isConfiguringController = (isNewController) or (not isNewController and (input(f"Do you want to change controller configuration for {joystick.get_name()}? Y = yes, not y = no? -->").lower() == "y"))
        if isConfiguringController:
            ...
            for i in range(len(button_names)):
                ...
                wasPreviouslyPushingAxis = False
                isPushingAxis = False
